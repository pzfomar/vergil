package com.pzfomar.vergil.utility;
...
public class JwtTokenProvider {
    @Value("${jwt.secretKey:rzxlszyykpbgqcflzxsqcysyhljt}")
    private String jwtSecretKey;

    @Value("${jwt.validityInMs:3600000}")
    ...
    @PostConstruct
    public void init() {
        var secret = Base64.getEncoder().encodeToString(this.jwtSecretKey.getBytes());
    ...
    public String createToken(Authentication authentication) {
        ...
        Date now = new Date();
        Date validity = new Date(now.getTime() + this.jwtValidityInMs);
